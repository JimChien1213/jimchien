...
% The configuration parameters of a DIT project. Each of these parameters
% can be later overwritten in DIT.m

%% 1. Scenario settings
% General
conf.project = 'Oman'; % The project name (same as project's folder name)
conf.saveas = 'Test 1'; % The suffix of output files (e.g. MaizeOman)
conf.SimStart = datetime(2000,01,01); % Simulation starting date [yyyy,mm,dd]
conf.SimEnd = datetime(2099,12,31); % Simulation ending date [yyyy,mm,dd]
...
conf.hist_same_xaxis = true; % Set the same x-axis scale to each histogram 
...
conf.crop.climate_file = 'Climate/weather_Seeb_100y.txt'; % Climatic database (*.txt)
...
% 3.3 AOS settings
% Find more info about AOS files in /Instructions/AquaCropOSv50a_ReferenceManual_Jan2017.pdf
conf.AOS.crop_file = 'Crop/Crop_MaizeGDD.txt'; % Crop type
conf.AOS.crop_irr_file = 'Crop/IrrigationManagement.txt'; % Irrigation management
conf.AOS.cropmix_file = 'Crop/CropMix.txt';
conf.AOS.CO2_file = 'Climate/MaunaLoaCO2.txt'; % CO2 projection
conf.AOS.field_file = 'Crop/FieldManagement.txt'; % Field Management
conf.AOS.soil_file = 'Soil/Soil.txt'; % Soil
conf.AOS.soil_profile =  'Soil/SoilProfile.txt'; % Soil profile
% conf.AOS.soil_texture_file = 'Soil/AOS_SoilTexture_SandyClayLoam.txt'; % Soil file (if needed -> switch on "CalcSHP" in soil_file)
conf.AOS.soil_hyrdo_file = 'Soil/SoilHydrology.txt'; % Soil properties  !Overwritten if conf.combination = 2 or 3)
conf.AOS.soil_iniWC_file = 'Soil/InitialWaterContent.txt'; % Initial Water Content
...
conf.soil.name = 'Soil/SoilGen/AOS_SoilHydrology_'; % Name of the soil file without last number (for iteration)
...
conf.irr.storage = [200 400 600]; % Water storage(s) [mm/season]      !Will be sorted and duplicates removed
