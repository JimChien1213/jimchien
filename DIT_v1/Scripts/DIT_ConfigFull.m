...
% can be later overwritten in the additional configuration file of your project and/or DIT.m

%% 1. Scenario settings
% General
conf.project = 'Example'; % The project name (same as project's folder name)
...
conf.crop.climate_file = 'Climate/Climate_100years.txt'; % Climatic database (*.txt)
...
conf.AOS.soil_hyrdo_file = 'Soil/AOS_SoilHydrSology.txt'; % Soil properties  !Overwritten if conf.combination = 2 or 3)
...
% If you enter multiple storages, a Crop-Water Production Function(CWPF) will be activated  
conf.irr.storage = [50 100 200]; % Water storage(s) [mm/season]      !Will be sorted and duplicates removed
...
conf.irr.strategy = 1;
    ...
    % ---- With optimization ----    
        % 5 - Optimized deficit irrigation with decision table
        % 6 - Optimized deficit irrigation with decision table (considering 4 phenological stages)
        % 7 - Optimized deficit irrigation with GET-OPTIS

% 4.3 Irrigation optimization (strategies 5-7)
% Optimizer setup
