...
%     EsPotMax = Soil.Kex*Et0*(1-NewCond.CCxW*(Soil.fwcc/100));
    EsPotMax = Soil.Kex*(1-NewCond.CCxW*Soil.fwcc/100)*(1-NewCond.CCadj)*Et0;
...
%         EsPotMin = Soil.Kex*(1-CCxActAdj)*Et0;
        EsPot = EsPot*(1-NewCond.CCxAct*(Soil.fwcc/100)*mult);
        EsPotMin = Soil.Kex*(1-NewCond.CCxAct*(Soil.fwcc/100)*mult)*(1-1.72*NewCond.CCxAct+NewCond.CCxAct^2-0.3*NewCond.CCxAct^3)*Et0;
        if EsPotMin < 0
            EsPotMin = 0;
        end
        if EsPot < EsPotMin
...
%% Stage 2 evaporation %%
% Extract water
if ToExtract > 0 && not(isempty(NewCond.Wstage2))
    % Start stage 2
    NewCond.Stage2 = true;
