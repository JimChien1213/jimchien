function [ParamStruct,CropChoices,FileLocation] = AOS_ReadModelParameters(FileLocation,conf) % CHANGED CODE
...
FileLocation.SoilProfileFilename = conf.AOS.soil_profile; % NEW CODE SoilProfileName{:}{:};
FileLocation.SoilTextureFilename = conf.AOS.soil_texture_file; % NEW CODE SoilTextureName{:}{:};
FileLocation.SoilHydrologyFilename = conf.AOS.soil_hyrdo_file; % NEW CODE SoilHydrologyName{:}{:};
...
for ii = 1:nCrops
    % Open file
    filename = strcat(Location,conf.AOS.crop_file); % CHANGED CODE
    CropInfo{1,1}{ii} = conf.crop.name; % NEW CODE
    fileID = fopen(filename);
    if fileID == -1
        % Can't find text file defining crop mix parameters
        % Throw error message
    ...
    ParamStruct.Crop.(CropInfo{1,1}{ii}).PlantingDate = conf.crop.SeedingDate; % NEW CODE
    ParamStruct.Crop.(CropInfo{1,1}{ii}).HarvestDate = conf.crop.HarvestDate; % NEW CODE
    
    % Add irrigation management information
    ParamStruct.Crop.(CropInfo{1,1}{ii}).IrrigationFile = conf.AOS.crop_irr_file; % CropInfo{1,3}{ii};
end
