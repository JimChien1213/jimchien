function [] = AOS_Initialize(WeatherStruct,conf) % CHANGED CODE
...
%% Get file locations %%
FileLocation = AOS_ReadFileLocations(conf);
FileLocation.OutputFilename = 'AOS'; % NEW CODE
FileLocation.WriteDaily = 'Y';
%if conf.save_data, FileLocation.WriteDaily = 'Y'; else, FileLocation.WriteDaily = 'N'; end % NEW CODE

%% Define model run time %%
%AOS_ClockStruct = AOS_ReadClockParameters(FileLocation);
AOS_ClockStruct = AOS_ReadClockParameters(conf); % NEW CODE

%% Read climate data %%
%WeatherStruct = AOS_ReadWeatherInputs(FileLocation);

%% Read model parameter files %%
[ParamStruct,CropChoices,FileLocation] = AOS_ReadModelParameters(FileLocation,conf);
...
GwStruct = AOS_ReadGroundwaterTable(FileLocation, conf.AOS.soil_GW_file); % CHANGED CODE
...
InitCondStruct.Pcum = 0; % NEW CODE
InitCondStruct.ETcum = 0; % NEW CODE
...
AOS_InitialiseStruct.Outputs.FinalOutput = cell(AOS_ClockStruct.nSeasons,10);
...
if strcmp(AOS_InitialiseStruct.FileLocation.WriteDaily,'Y')
    ...
    fprintf(fid,strcat(repmat('%s\t',1,10),'\n'),'GSeason','Crop','PlantD',...
        'PlantSD','HarvestCD','HarvestSD','Yield','TotIrr','TotP','TotET'); % CHANGED CODE
    fclose(fid);
end
